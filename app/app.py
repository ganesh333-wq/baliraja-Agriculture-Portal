...
if not os.path.exists(disease_model_path):
    raise FileNotFoundError(f"Missing file: {disease_model_path}")
if not os.path.exists(crop_recommendation_model_path):
    raise FileNotFoundError(f"Missing file: {crop_recommendation_model_path}")

# Load disease model
disease_model = ResNet9(3, len(disease_classes))
disease_model.load_state_dict(torch.load(
    disease_model_path, map_location=torch.device('cpu'), weights_only=True  # safer
))
disease_model.eval()
...
@app.route('/')
def home():
    return render_template('index.html', title='Baliraja - Home')

# Crop recommendation page
@app.route('/crop-recommend')
def crop_recommend():
    return render_template('crop.html', title='Baliraja - Crop Recommendation')

# Fertilizer recommendation page
@app.route('/fertilizer')
def fertilizer_recommendation():
    return render_template('fertilizer.html', title='Baliraja - Fertilizer Suggestion')
...
def disease_prediction():
    if request.method == 'POST':
        ...
        if not file:
            return render_template('disease.html', title='Baliraja - Disease Detection')
        try:
            img = file.read()
